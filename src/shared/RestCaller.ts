...
export const updateEntity = async (endpoint: string, id: string, body: string) => {
    try {
        const requestOptions = {
            method: 'PUT',
            headers: {
        ...
        const response = (await fetch(`${import.meta.env.VITE_REQUEST_URL}/${endpoint}/${id}`, requestOptions));
        return await response.json();
...
export const deleteEntity = async (endpoint: string, id: string) => {
    try {
        ...
        await fetch(`${import.meta.env.VITE_REQUEST_URL}/${endpoint}/${id}`, requestOptions);
