...
export default function AcademicTab({
    ...
    return (<>
        <div key={`${academic.id}`} className={"flex flex-col items-start careerTileWidth h-auto mt-5 mr-5"}>
            {isEditVisible &&
              <div className={"flex flex-row"}>
                <FaMinus className={"hover:text-green-600 cursor-pointer mr-2 mb-2"}
                         onClick={() => onDelete(academic.id)}/>
                <EditStepModal onSaveExp={onSaveEditedCareer} editExpObj={academicDt}
                               titleModal={"Bearbeiten"}/>
              </div>
            }
            <p className={"text-xl font-bold mb-2 text-green-600"}>{academic.title}</p>
            <p className={"text-sm font-bold"}>{academic.school} von {formattedFromDate.getMonth()}/{formattedFromDate.getFullYear()} bis {formattedToDate.getMonth()}/{formattedToDate.getFullYear()}</p>
            <ul className={"flex flex-col items-start text-sm"}>{academic.focusList.map((focus, index) => <li
